...
class TestDataset(unittest.TestCase):
    def test_build_mtl_dataset(self):
        ...
        mtl_dataset = build_mtl_dataset(datasets, task_ids_map, stopping_strategy=None)
    ...
    def test_build_mtl_dataset_interleave_f_exhausted(self):
        ...
        assert len(mtl_dataset) == min(len(ds) for ds in datasets.values()) * len(
            datasets
        )
    ...
    def test_build_mtl_dataset_interleave_all_exhausted(self):
        ...
        assert len(mtl_dataset) == max(len(ds) for ds in datasets.values()) * len(
            datasets
        )
